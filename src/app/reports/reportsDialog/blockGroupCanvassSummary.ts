...
@Component({
  templateUrl: './blockGroupCanvassSummary.html',
})

export class BlockGroupCanvassSummary implements OnInit{
  ...
  constructor(public dialogRef: MatDialogRef<BlockGroupCanvassSummary>,
  ...
  async getAllReports() {
      ...
      this.orgService.getCampaignOrgs(campaignID).subscribe(async orgs => {
          ...
          for (let i in orgs) {
              this.reportService.getBlockGroupCanvassSummaryReport(campaignID, orgs[i]._id).subscribe(async (reports: any[]) => {
                  if (reports) {
