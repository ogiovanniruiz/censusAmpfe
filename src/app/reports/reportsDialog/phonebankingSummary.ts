...
@Component({
    templateUrl: './phonebankingSummary.html',
})

export class PhonebankingSummary implements OnInit{
    ...
    constructor(public dialogRef: MatDialogRef<PhonebankingSummary>,
    ...
    async getAllReports() {
        ...
        /*await this.storage.get('phonebankingSummaryTime').subscribe(async (phonebankingSummaryTime: []) => {
                        ...
                        this.reportService.getPhonebankingSummaryReport(campaignID, orgs[i]._id, orgs[i].name).subscribe(
                            async (reports: any[]) => {
                                ...
                                if (this.dataLoadedValue === 100) {
                                    ...
                                    //this.storage.set('phonebankingSummary', this.allReports).subscribe(() => {});
                                }
                            }
                        );
                    }
            ...
            /*} else {
                this.checkPhonebankingSummary();
            }
        });*/
    }

    checkPhonebankingSummary() {
        this.storage.get('phonebankingSummary').subscribe(async (phonebankingSummary: []) => {
            if (phonebankingSummary != null) {
                this.allReports = phonebankingSummary;
                this.sortedReports = this.allReports.slice();
                this.totalSize = this.allReports.length
                this.iterator();
                this.dataLoaded = false;
            } else {
                setTimeout(() => this.checkPhonebankingSummary(), 5000);
            }
        });
    }
