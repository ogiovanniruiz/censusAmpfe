...
@Component({
  templateUrl: './petitionSummary.html',
})

export class PetitionSummary implements OnInit{
  ...
  constructor(public dialogRef: MatDialogRef<PetitionSummary>,
  ...
  async getAllReports() {
    ...
    //await this.storage.get('petitionSummaryTime').subscribe(async (petitionSummaryTime: []) => {
            ...
            this.reportService.getPetitionSummaryReport(campaignID, orgs[i]._id, orgs[i].name).subscribe(
                async (reports: any[]) => {
                  ...
                  if (this.dataLoadedValue === 100) {
                    ...
                    this.storage.set('petitionSummary', this.allReports).subscribe(() => {});
     ...
     // } else {
       // this.checkPetitionSummary();
      //}
    //});
  }

  checkPetitionSummary() {
    this.storage.get('petitionSummary').subscribe(async (petitionSummary: []) => {
      if (petitionSummary != null) {
        this.allReports = petitionSummary;
        this.sortedReports = this.allReports.slice();
        this.totalSize = this.allReports.length;
        this.iterator();
        this.dataLoaded = false;
      } else {
        setTimeout(() => this.checkPetitionSummary(), 5000);
      }
    });
  }
