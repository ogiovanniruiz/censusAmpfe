...
@Component({
  templateUrl: './blockGroupOrgSummary.html',
})

export class BlockGroupOrgSummary implements OnInit{
  ...
  constructor(public dialogRef: MatDialogRef<BlockGroupOrgSummary>,
  ...
  async getAllReports() {
    ...
    this.reportService.getBlockGroupOverallSummaryReport(campaignID, orgID).subscribe(async (reports: any[]) => {
      if (reports) {
